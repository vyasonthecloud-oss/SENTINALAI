...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-redirect to /admin if user is admin, or use redirect parameter
      const targetDestination = redirectParam || (data.user.role === 'ADMIN' ? '/admin' : '/');
  ...
  const handleGoogleLogin = () => {
    window.location.href = `/api/auth/google${redirectParam ? `?redirect=${encodeURIComponent(redirectParam)}` : ''}`;
  };
  ...
  return (
    <div className="w-full max-w-md relative z-10">
      ...
      <div className="text-center mb-10">
        <Link href="/" className="inline-block mb-6">
          <div className="w-16 h-16 bg-primary/20 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(25,25,112,0.3)] mx-auto border border-primary/30">
            <span className="font-black text-2xl text-gradient">S</span>
          </div>
        </Link>
      ...
      <div className="glass rounded-3xl p-8 card-glow border border-border/80 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3.5 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 group disabled:opacity-70 cursor-pointer"
          >
        ...
        <div className="mt-6 space-y-3">
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full bg-card hover:bg-muted border border-border text-foreground font-semibold py-3 rounded-xl transition-all flex items-center justify-center gap-3 cursor-pointer"
          >
      ...
      <p className="text-center text-sm text-muted-foreground mt-8">
        ...
        <Link href={`/signup${redirectParam ? `?redirect=${redirectParam}` : ''}`} className="font-bold text-primary hover:text-primary/80 transition-colors">
          Sign up now
        </Link>
      </p>
    </div>
